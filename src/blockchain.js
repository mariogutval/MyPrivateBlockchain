  ...
  validateChain() {
    ...
    return new Promise(async (resolve, reject) => {
      self.chain.forEach((block) => {
        if(block.validate() == 'The block is not valid'){
          errorLog.push({blockNumber: block.height, error: 'Validation failed.'});
        }
        else if(block.previousBlockHash != self.chain[block.height - 1].hash){
          errorLog.push({blockNumber: block.height, error: 'Previous hash does not match.'});    
		}
      });
      resolve(errorLog);
    });
